...
The **putDown()** method of the Kid object is successfully tested. It will dynamically test for all kids on the list, even if additional kids are coded into the list.
Other object method tests were done manually due to the complexity of the data structure and the complexity of testing an object status that is constantly changing. Upon completion of the course I intend to revisit my Jasmine testing suite to find a better solution that is easier to implement.
